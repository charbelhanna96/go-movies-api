...
func newRequest(params map[string]string) *http.Request {
	...
	req, _ := http.NewRequest("GET", "/api/v1/movies?"+query.Encode(), nil)
	return req
}
...
func TestParseMovieFilters_ValidDirectors(t *testing.T) {
	req := newRequest(map[string]string{"directors": "1,2,3"})
	filters, err := validator.ParseMovieFilters(req)
	require.NoError(t, err)
...
func TestParseMovieFilters_ValidGenres(t *testing.T) {
	req := newRequest(map[string]string{"genres": "1,5"})
	filters, err := validator.ParseMovieFilters(req)
	require.NoError(t, err)
...
func TestParseMovieFilters_ValidYearRange(t *testing.T) {
	req := newRequest(map[string]string{"min-year": "2000", "max-year": "2020"})
	filters, err := validator.ParseMovieFilters(req)
	require.NoError(t, err)
...
func TestParseMovieFilters_ValidDurationRange(t *testing.T) {
	req := newRequest(map[string]string{"min-duration": "90", "max-duration": "180"})
	filters, err := validator.ParseMovieFilters(req)
	require.NoError(t, err)
...
func TestParseMovieFilters_ValidRatingRange(t *testing.T) {
	req := newRequest(map[string]string{"min-rating": "4.0", "max-rating": "5.0"})
	filters, err := validator.ParseMovieFilters(req)
	require.NoError(t, err)
...
func TestParseMovieFilters_ValidLimit(t *testing.T) {
	req := newRequest(map[string]string{"limit": "10"})
	filters, err := validator.ParseMovieFilters(req)
	require.NoError(t, err)
...
func TestParseMovieFilters_DeduplicatesDirectorIDs(t *testing.T) {
	req := newRequest(map[string]string{"directors": "1,2,1,3"})
	filters, err := validator.ParseMovieFilters(req)
	require.NoError(t, err)
	assert.Equal(t, []int{1, 2, 3}, filters.DirectorIDs)
}
...
func TestParseMovieFilters_TooManyIDs(t *testing.T) {
	...
	req := newRequest(map[string]string{"directors": raw})
	_, err := validator.ParseMovieFilters(req)
	assert.Error(t, err)
}

func TestParseMovieFilters_ZeroDirectorID(t *testing.T) {
	req := newRequest(map[string]string{"directors": "0"})
	_, err := validator.ParseMovieFilters(req)
	assert.Error(t, err)
}

func TestParseMovieFilters_NegativeDirectorID(t *testing.T) {
	req := newRequest(map[string]string{"directors": "-1"})
	_, err := validator.ParseMovieFilters(req)
	assert.Error(t, err)
}

func TestParseMovieFilters_LimitAboveMaximum(t *testing.T) {
	req := newRequest(map[string]string{"limit": "10001"})
	_, err := validator.ParseMovieFilters(req)
	assert.Error(t, err)
}

func TestParseMovieFilters_DurationAboveMaximum(t *testing.T) {
	req := newRequest(map[string]string{"min-duration": "601"})
	_, err := validator.ParseMovieFilters(req)
	assert.Error(t, err)
}
